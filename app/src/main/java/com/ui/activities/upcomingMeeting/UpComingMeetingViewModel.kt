...
class UpComingMeetingViewModel @Inject constructor(
    ...
    fun clearObserverList()
    {
        scheduledMeetingLiveData.postValue(arrayListOf())
    }
        ...
        CoroutineScope(Dispatchers.IO + exceptionHandler)
            .launch {
                try {
                    //val authToken= DataStoreHelper.getLoginBearerToken()
                    // Log.d("TAG", "getQuestionnaireList: token is this $authToken")
                    val response = baseRepoApi.getSMSchatHistory(DataStoreHelper.getLoginBearerToken(),BodySMSchatHistory(candidateId.toInt()))
        ...
        CoroutineScope(Dispatchers.IO + exceptionHandler)
            .launch {
                try {
                    ...
                    val response = baseRepoApi.getQuestionnaireForCandidate("/api/Questionier/GetCandidateQuestionierAnswerSheet/"+candidateId)
